...
module.exports.getSub = function () {
    return sub;
}
...
module.exports.Server = function (modal, initCallback) {
        ...
        try {
            ...
            if (cluster) {
                sub = redis.createClient(modal.redis.port, modal.redis.host, {
                    auth_pass: modal.redis.password
                });
    ...
    function initListening(dbData) {
        ...
        app.post("/getUserTags", async function (req, res) {
            try {
                res.json(await bot.readTags({
                    webId: req.body.sender
                }, dbData))
            } catch (e) {
                console.log(e)
                res.sendStatus(500)
            ...
            sub.on("message", async function (channel, message) {
                if (channel == "default-web-external") {
                    ...
                    if (socketIdMapForBot[data.sender.toString()] && socketObjectMap[socketIdMapForBot[data.sender.toString()]]) {
                        socketObjectMap[socketIdMapForBot[data.sender.toString()]].emit("web-external-" + data.sender.toString(), {
                            text: data.text,
                            type: data.type,
                            next: data.next
                        })
                    ...
                    if (socketIdMapForBot[data.sender.toString()] && socketObjectMap[socketIdMapForBot[data.sender.toString()]]) {
                        socketObjectMap[socketIdMapForBot[data.sender.toString()]].emit("web-webview-" + data.sender.toString(), {
                            text: data.text
                        })
    ...
    function botProtocols(socket, dbData) {
        ...
        socket.on("web-save", async function (data) {
            try {
                ...
                console.log(e)
                socket.emit("web-save-" + data.webId + "-" + data.requestId, {
                    error: e
                })
            }
        })
        socket.on("web-pre", async function (data) {
            try {
                ...
                console.log(e)
                socket.emit("web-pre-" + data.webId + "-" + data.requestId, {
                    error: e
                })
            }
        })
        socket.on("web-post", async function (data) {
            try {
                ...
                console.log(e)
                socket.emit("web-post-" + data.webId + "-" + data.requestId, {
                    error: e
                })
            }
        })
